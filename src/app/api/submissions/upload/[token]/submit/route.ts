...
export async function POST(
  ...
  try {
    ...
    const { data: clientFromToken, error: clientFromTokenError } = await supabase
      .from('clients')
      .select('id, name')
      .eq('upload_token', params.token)
      .maybeSingle()
    
    console.log('Client lookup result:', { hasClient: !!clientFromToken, error: clientFromTokenError })
    ...
    if (hasPreGeneratedCaptions) {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        for (const captionData of imageCaptions) {
          ...
          // Instagram supports up to 2,200 characters - no truncation needed
          // Just ensure brief text is in the first 125 chars (before "more" cutoff)
